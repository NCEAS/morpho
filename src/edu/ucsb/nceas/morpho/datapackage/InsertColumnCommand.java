...
/**
 * Class to handle insert a column command
 */
public class InsertColumnCommand implements Command 
{
  ...
  /* Reference to the Attribute Dialog */
...
//         String fieldDelimiter=dataView.getFieldDelimiter();
    ...
    if (sel>-1) 
    {
       ...
       if (columnAddFlag) 
       {
		...
		insertNewAttributeAt(sel, attributeDoc);
           ...
           {
		JOptionPane.showMessageDialog(morphoFrame, "calling insert Attr", "Message",JOptionPane.INFORMATION_MESSAGE, null);
  ...
  void insertNewAttributeAt(int index, Document doc) 
  {
	  ...
	  /*Node fNode = doc.createElement("test");
	  newText = doc.createTextNode(XMLUtil.normalize("hello"));
	  fNode.appendChild(newText);*/
  ...
  private void showAttributeDialog() {
	...
	attributePage = (AttributePage)DataPackageWizardPlugin.getPage(DataPackageWizardInterface.ATTRIBUTE_PAGE);
	WizardPopupDialog wpd = new WizardPopupDialog(attributePage, mf);
