 ...
 */
public class EditDataReplicationPolicyCommand implements Command,
	...
	public EditDataReplicationPolicyCommand() {
	}
	...
	public void wizardComplete(Node newDOM, String autoSavedID) {
		...
		if (morphoFrame != null) {
			DataViewContainerPanel resultPane = morphoFrame.getDataViewContainerPanel();
		...
		if (showDialog()) {
			...
			String identifier = mdp.getAbstractDataPackage().getEntity(entityIndex).getIdentifier().getValue();
			...
			try {
				...
				// save SM independently from data file
				success = DataStoreServiceController.getInstance().setReplicationPolicy(mdp.getAbstractDataPackage().getEntity(entityIndex), mdp.getAbstractDataPackage().getLocation());
	...
	private boolean showDialog() {
		...
		// set the existing replication policy for the data
		replicationPolicyPage.setReplicationPolicy(entity.getSystemMetadata().getReplicationPolicy());
