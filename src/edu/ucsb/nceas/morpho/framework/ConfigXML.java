 ...
 *     Authors: Dan Higgins
 *
 *     Version: '$Id: ConfigXML.java,v 1.1 2000-11-20 17:45:51 higgins Exp $'
 */
 ...
 //   out = new PrintWriter(new OutputStreamWriter(System.out));
    ...
    */
    public String get(String key, int i) {
        ...
        String result = null;
        if (nl.getLength()<1) {return result;}
        if (nl.getLength()<i) {return result;}
        Node cn = nl.item(i).getFirstChild();  // assume 1st child is text node
        if ((cn!=null)&&(cn.getNodeType()==Node.TEXT_NODE)) {
 ...
 public Hashtable getHashtable(String parentName, String keyName, String valueName) {
    ...
    if (nl.getLength()>0) {
        ...
        for (int j=0;j<children.getLength();j++) {
            ...
            if ((cn.getNodeType()==Node.ELEMENT_NODE)&&(cn.getNodeName().equalsIgnoreCase(keyName))) {
                Node ccn = cn.getFirstChild(); // assumed to be a text node
                if ((ccn!=null)&&(ccn.getNodeType()==Node.TEXT_NODE)) {
                    keyval = ccn.getNodeValue();
                }}
            if ((cn.getNodeType()==Node.ELEMENT_NODE)&&(cn.getNodeName().equalsIgnoreCase(valueName))) {
                Node ccn = cn.getFirstChild(); // assumed to be a text node
                if ((ccn!=null)&&(ccn.getNodeType()==Node.TEXT_NODE)) {
